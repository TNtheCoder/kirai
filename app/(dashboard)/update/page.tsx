...
const UpdatePage = () => {
  ...
  return (
    <div className=" bg-black overflow-x-auto flex flex-col items-center min-h-screen mt-24 px-10">
  ...
  {Array.isArray(data) &&
    data.map((tree, index) => (
      <tr
        ...
        } `} // Alternate row colors and hover effect
      >
        ...
        <td className="px-4 py-2 border-b">
          {new Date(tree.timestamp).toLocaleString()}
        </td>
        <td className="px-4 py-2 border-b">
          {isLoaded && userId ? (
            <Button
              variant="default"
              size="sm"
              onClick={() => router.push(`/update/${tree.id}`)}
              className="text-white bg-gray-700 hover:bg-gray-600"
            >
              Update
            </Button>
          ) : (
            <p className="text-gray-400 text-sm">Sign in to update</p>
          )}
        </td>
      </tr>
    ))}
